...
#define	FLASH_DRAIN_TIME	 1000.2 //100 units/3 minutes // was 1.2
...
// int gmsgFlashBattery = 0; FragMented
int gmsgResetHUD = 0;
int gmsgInitHUD = 0;
...
int gmsgHealth = 0;
int gmsgDamage = 0;
//int gmsgBattery = 0; FragMented
//int gmsgTrain = 0; FragMented
int gmsgLogo = 0;
...
int gmsgGameMode = 0;
int gmsgMOTD = 0;
int gmsgVGUIMenu = 0; // FragMented
int gmsgVGUIClasses = 0; // FragMented
...
int gmsgSetTimer = 0;
int gmsgRallyScoreInfo = 0;
int gmsgRaceInfo = 0;
int gmsgFixAngle = 0;
int gmsgCheckpoint = 0;
int gmsgLapEnd = 0;
int gmsgRaceEnd = 0;
int gmsgHudRadar = 0;
...
//	gmsgFlashBattery = REG_USER_MSG("FlashBat", 1); FragMented
	gmsgHealth = REG_USER_MSG( "Health", 1 );
	gmsgDamage = REG_USER_MSG( "Damage", 12 );
//	gmsgBattery = REG_USER_MSG( "Battery", 2); FragMented
//	gmsgTrain = REG_USER_MSG( "Train", 1); FragMented
	...
	// SaRcaZm - V8 - Start
	gmsgScoreInfo = REG_USER_MSG( "ScoreInfo", 4 );
	// SaRcaZm - V8 - End
	...
	// SaRcaZm - V8 - Start
	// One parameter: 1) VGUI menu number
	gmsgVGUIMenu = REG_USER_MSG(hlConst.szMsg_VGUIMenu, 1);
	// One parameter: 1) Team index
	gmsgVGUIClasses = REG_USER_MSG(hlConst.szMsg_VGUIClasses, -1);
	// Two parameters: 1) Gloabal Start Time;
	gmsgSetTimer = REG_USER_MSG (hlConst.szMsg_SetTimer, 2);
	// Two parameters: 1) Total laps; 2) Total checkpoints / lap; 3) Number of players on server;
	//				   4) Start time for time extension
	gmsgRaceInfo = REG_USER_MSG (hlConst.szMsg_RaceInfo, 4);
	// No parameters
	gmsgFixAngle = REG_USER_MSG (hlConst.szMsg_FixAngle, 0);
	// Two parameters: 1) Player index; 2) Current place; 3) Time extension time
	gmsgCheckpoint = REG_USER_MSG (hlConst.szMsg_Checkpoint, 3);
	// Three parameters: 1) Player index; 2) Current place; 3) Lap time; 4) Time extension time
	gmsgLapEnd = REG_USER_MSG (hlConst.szMsg_LapEnd, 5);	// Coords take up 2 places
	// Four parameters: 1) Player index; 2) Current place; 3) Lap time; 4) Race time
	gmsgRaceEnd = REG_USER_MSG (hlConst.szMsg_RaceEnd, 6);
	// Two parametrs: 1) Player index; 2) Boolean spectator on?
	gmsgSpectator = REG_USER_MSG(hlConst.szMsg_Spectator, 2 );
	// Two parameters: 1) Player index; 2) Score
	// This is to save sending a ScoreInfo message to just update the score
	gmsgRallyScoreInfo = REG_USER_MSG (hlConst.szMsg_RallyScoreInfo, 3);
	// One parameter: 1) Global Start Time
	gmsgCountdown = REG_USER_MSG ("Countdown", 2);	// TODO: Maybe move this to Rally_Antihaxor?
	// One parameter: 1) Text to send
	gmsgTextFiles = REG_USER_MSG( "TextFiles", -1 );
	// One parameter: 1) Amount of money
	gmsgMoney = REG_USER_MSG ("Money", 2);
	// One parameter: 1) Player index;
	gmsgHorn = REG_USER_MSG ("Horn", 1);
	// Three parameters: 1) vuser4[0]; 2) vuser4[1]; 3) vuser4[2]
	gmsgUserStats = REG_USER_MSG ("UserStats", 6);
	// SaRcaZm - V8 - End
	
	// Creme: 11-Apr-04 Radar added
...
void CBasePlayer :: ShowVGUIClassMenu( void )
{
	
	MESSAGE_BEGIN(MSG_ONE, gmsgVGUIClasses, NULL, pev);
...
// SaRcaZm - V6 - Start
// Money stuff
#define MAX_AMOUNT	7000

bool AllowMoney (void)
{
	return true;	// DEBUG purposes

	// No money if they are the only one
	int numplayers = 0;
...
void CBasePlayer::AddMoney (int amount)
{
	// SaRcaZm - V8 - Start
	if (!AllowMoney ())
		return;
	...
	// Cap the amount
	if (pev->vuser3[0] > MAX_AMOUNT)
		pev->vuser3[0] = MAX_AMOUNT;
}

// Returns false if there isn't enough money
bool CBasePlayer::RemoveMoney (int amount)
{
	// SaRcaZm - V8 - Start
	if (!AllowMoney ())
		return true;
...
void CBasePlayer::UpdateMoney (void)
{
	// SaRcaZm - V8 - Start
	if (!AllowMoney ())
		return;
	...
	// Update the amount on the client side
	MESSAGE_BEGIN( MSG_ONE, gmsgMoney, NULL, pev );
		WRITE_SHORT((int)pev->vuser3[0]);
	MESSAGE_END();
}

void CBasePlayer::ResetMoney (void)
{
	pev->vuser3[0] = 0;
}
// SaRcaZm - V6 - End
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// SaRcaZm - V4 - Start
	// Dying all the time is pisssing me off
	return 0;
	// SaRcaZm - V4 - End
...
/*creme	if (pev->air_finished < gpGlobals->time)
...
/*crem if (!RANDOM_LONG(0,0x1f) && RANDOM_LONG(0,AIRTIME-1) >= air)
	{
...
// wait for any button down,  or mp_forcerespawn is set and the respawn time is up
	// SaRcaZm - V8 - Start
	if (g_pGameRules->m_RoundState == 3 && pev->fuser1)
		fAnyButtonDown = true;
	// SaRcaZm - V8 - End
...
void CBasePlayer::AddPoints( int score, BOOL bAllowNegativeScore )
{
	...
	// SaRcaZm - V8 - Start
	// Too much info was being sent with this message
	MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		...
		//WRITE_SHORT( 0 );
		//WRITE_SHORT( g_pGameRules->GetTeamIndex( m_szTeamName ) + 1 );
		WRITE_BYTE( g_pGameRules->GetTeamIndex( m_szTeamName ) + 1 );
	MESSAGE_END();
	// SaRcaZm - V8 - End
}
...
void CBasePlayer::PreThink(void)
{
	...
	{
		// SaRcaZm - V8 - Start
		// If we aren't watching anyone, search for a new target
		if (!pev->iuser2)
...
// SaRcaZm - V7 - Start
void CBasePlayer::Rally_ChangeModel (const char *szModel)
{
	strcpy (m_szModel, szModel);	// Save for later checking
	g_engfuncs.pfnSetClientKeyValue( entindex(), g_engfuncs.pfnGetInfoKeyBuffer (edict ()), "model", m_szModel );
}
// SaRcaZm - V7 - End
...
void CBasePlayer::PostThink()
{
	...
	// SaRcaZm - V7 - Start
	// Prevent any unwanted model switching
	char *info = g_engfuncs.pfnGetInfoKeyBuffer (edict ());
	if (!strstr (info, m_szModel))
		g_engfuncs.pfnSetClientKeyValue( entindex(), info, "model", m_szModel );
	// SaRcaZm - V7 - End
	...
	if (gpGlobals->time > m_iRadarUpdate)
	{
 		// Count the number of players
		for ( j = 1; j <= gpGlobals->maxClients; j++ )
 		{
 			CBasePlayer *plr = (CBasePlayer *)UTIL_PlayerByIndex( j );;
 			if (( plr ) && ( plr != this ) && plr->pev && plr->IsAlive())
 			{
				// In the map, so add it to array .. very simple algorithm to convert to byte
				iTargetRight[iTargetNumber] = int(plr->pev->origin[0] / 32) + 128;
				iTargetForward[iTargetNumber] = int(plr->pev->origin[1] / 32) + 128;
		...
		//ALERT( at_console, "Sending Target Info: %i count: %i\n", iTargetNumber, count); 

		// Send coordinates to CHudRadar to draw
		// Set the message parameters - what type of msg + where to send 
		MESSAGE_BEGIN( MSG_ONE, gmsgHudRadar, NULL, pev ); 
		WRITE_BYTE( iTargetNumber ); // total number of targets on the radar 

		for( i = 0; i < iTargetNumber; i++)
		{ 
			// send x, y coordinates
			// we could also send the target class type etc., 
			// if we wanted so that we could draw things in diff colours
			WRITE_BYTE( iTargetRight[i] );
			WRITE_BYTE( iTargetForward[i] );
		}

		MESSAGE_END();

		m_iRadarUpdate = gpGlobals->time + 0.5;
	}
...
// END RADAR CODE
	...
	if (pev->fixangle)
	{
		// Send the message to the client so the yaw is not fixed
		MESSAGE_BEGIN (MSG_ONE, gmsgFixAngle, pev->origin, pev);
		MESSAGE_END ();
	}
	// SaRcaZm - V2 - End
	// SaRcaZm - V8 - Start
	UTIL_SetSize( pev, Vector(0, 0, 0), Vector(0, 0, 0));	// Bug fix from Creme
	// SaRcaZm - V8 - End
...
BOOL IsSpawnPointValid( CBaseEntity *pPlayer, CBaseEntity *pSpot )
{
	...
	// SaRcaZm - V5 - Start
	// Players are being dropped to the floor, so that is where this trace should start
	TraceResult tr;
	vec3_t end = pSpot->pev->origin + Vector (0, 0, -2056);
	UTIL_TraceLine (pSpot->pev->origin, end, ignore_monsters, NULL, &tr);

	while ( (ent = UTIL_FindEntityInSphere( ent, tr.vecEndPos, 64 )) != NULL )
	// SaRcaZm - V5 - End
	{
...
// choose a info_player_deathmatch point
	...
	else if ( g_pGameRules->IsDeathmatch() )
	{
		...
		// SaRcaZm - V7 - Start
		// If they are looking at the VGUI, spawn them at an info_vgui_start
		// SaRcaZm - V8 - Start
		// This is done client side now
/*		if (plr->pev->iuser1 || !plr->pev->fuser1)
		{
			pSpot = UTIL_FindEntityByClassname( pSpot, "info_vgui_start" );
			if (!FNullEnt (pSpot))
			{
				goto ReturnSpot;
			}
...
*/		// SaRcaZm - V8 - End
		...
		// SaRcaZm - V6 - Start
		char forward_spots[2][40] = { "info_player_start", "info_player_deathmatch" };
		char reverse_spots[2][40] = { "info_player_start_reverse", "info_player_deathmatch_reverse" };

		for (int y = 0; y < 2; y++)
		{
			// All references to "info_player_deathmatch" below have been replaced with szSpotClass
			char *szSpotClass = (direction.value < 0) ? reverse_spots[y] : forward_spots[y];

			// Randomize the start spot
			for ( int i = RANDOM_LONG(1,5); i > 0; i-- )
				pSpot = UTIL_FindEntityByClassname( pSpot, szSpotClass );
			...
			do 
			{
				if ( pSpot )
				{
					...
					// SaRcaZm - V7 - Start
					// We have positions on the start points, so a valid spawn point is the correct position
					CBaseDMStart *pStart = (CBaseDMStart *)pSpot;
					if (plr->pev->iuser4 && pStart->m_iPosition)
					{
						// They have a position, so check it out
						if (plr->pev->iuser4 != pStart->m_iPosition)
						{
							pSpot = UTIL_FindEntityByClassname( pSpot, szSpotClass );
							continue;
						...
						{
							// Something went wrong - start again without looking for a specific position
							plr->pev->iuser4 = 0;
							pSpot = g_pLastSpawn;
							pSpot = UTIL_FindEntityByClassname( pSpot, szSpotClass );
							continue;
		...
		if ( !FNullEnt( pSpot ) )
		{
			CBaseEntity *ent = NULL;
			// SaRcaZm - V5 - Start
			// We are being dropped to the floor immediately after spawn, so kill anyone down there
			TraceResult tr;
			vec3_t end = pSpot->pev->origin + Vector (0, 0, -2056);
			UTIL_TraceLine (pSpot->pev->origin, end, ignore_monsters, NULL, &tr);
...
void CBasePlayer::Spawn( void )
{
	...
	pev->viewmodel		= MAKE_STRING("models/interior.mdl");
	// FragMented
	// SaRcaZm - V7 - Start
	if ((g_pGameRules->m_RoundState == ROUND_NOTSTARTED || g_pGameRules->m_RoundState == ROUND_END)	// Not started
		|| !pev->fuser1)																			// Not ready
	{
		pev->movetype	= MOVETYPE_NOCLIP;
		m_iHideHUD = HIDEHUD_ALL;	// Hide the HUD
		pev->effects = EF_NODRAW;	// Don't draw them
		EnableControl (false);	// Disable control so they can't move
	}
	else if (g_pGameRules->m_RoundState == ROUND_COUNTDOWN)
	{
		StopObserver ();
		EnableControl (true);
		pev->movetype = MOVETYPE_TOSS;
	...
	if (!*m_szTeamName)
		strcpy (m_szTeamName, "Spectator");

	fHornTime = gpGlobals->time;
	// SaRcaZm - V7 - End
...
// Copied from spawn() for now
	m_bloodColor	= DONT_BLEED; // FragMented
...
BOOL CBasePlayer :: FlashlightIsOn( void )
{
	...
	// SaRcaZm - Start
	// Temporary fix for headlights
	// For a temporary fix this stayed for quite a long time
...
void CBasePlayer :: FlashlightTurnOn( void )
{
	...
	// SaRcaZm - V5 - Start
	bFlashlightOn = true;
	MESSAGE_BEGIN (MSG_ALL, gmsgFlashlight);
		WRITE_BYTE (entindex());
		WRITE_BYTE (1);
	MESSAGE_END ();
	/*
...
/*	if (m_iTrain & TRAIN_NEW) FragMented
	{
		ASSERT( gmsgTrain > 0 );
		// send "health" update message
...
// SaRcaZm - V7 - Start
// Creme's camera code info spots
class CInfoCamera : public CPointEntity
{
	void Spawn (void)
	{
		UTIL_SetOrigin( pev, pev->origin );
...
LINK_ENTITY_TO_CLASS (info_camera, CInfoCamera);
// SaRcaZm - V7 - End
