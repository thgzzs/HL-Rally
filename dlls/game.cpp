...
// SaRcaZm - V8 - Start
cvar_t	direction = {"mp_direction", "1", FCVAR_ARCHIVE };
cvar_t	racemode = {"mp_racemode", "1", FCVAR_ARCHIVE };
//cvar_t	arcadetraction = {"mp_arcadetraction", "0", FCVAR_ARCHIVE};
cvar_t	numlaps	= {"mp_numlaps","2", FCVAR_ARCHIVE };
cvar_t	prerace	= {"mp_prerace","3", FCVAR_ARCHIVE };
cvar_t	joinmidrace = {"mp_joinmidrace","1", FCVAR_ARCHIVE};
cvar_t  allow_spectators = { "allow_spectators", "1.0", FCVAR_SERVER };		// 0 prevents players from being spectators
cvar_t	bot_skill = {"rally_bot_skill", "100", FCVAR_ARCHIVE};
cvar_t	hptime = {"rally_hptime", "20", FCVAR_ARCHIVE};
// SaRcaZm - V8 - End

// Creme: Added this to totally disable the stats for testing.
cvar_t	use_php_stats = {"sv_php_stats", "0", FCVAR_ARCHIVE };
...
{
	// SaRcaZm - V5 - Start
	// Load the anti-haxor data here
	Get_HL_Data (&hlConst);
	// SaRcaZm - V5 - End
	// SaRcaZm - V8 - Start
	g_engfuncs.pfnAddServerCommand ("addbot", BotCreate);
	g_engfuncs.pfnAddServerCommand ("race_restart", RaceRestart);
	// SaRcaZm - V8 - End
...
// END REGISTER CVARS FOR SKILL LEVEL STUFF
	...
	// SaRcaZm - V8 - Start
	CVAR_REGISTER (&numlaps);
	CVAR_REGISTER (&prerace);
	CVAR_REGISTER (&joinmidrace);
	CVAR_REGISTER( &allow_spectators );
	CVAR_REGISTER (&direction);
	CVAR_REGISTER (&racemode);
	CVAR_REGISTER (&use_php_stats);
	//CVAR_REGISTER (&arcadetraction);
	CVAR_REGISTER (&bot_skill);
	CVAR_REGISTER (&hptime);
	// SaRcaZm - V8 - End
