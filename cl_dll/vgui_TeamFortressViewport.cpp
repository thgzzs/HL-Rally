...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	SchemeHandle_t hPrimaryScheme = m_SchemeManager.getSchemeHandle( "Primary Button Text" );
	{
		...
		// Too much orange...a nice medium gray instead
		pScheme->setColor(Scheme::sc_secondary1, 128, 128, 128, 0);
		// SaRcaZm - V7 - End
	}
...
// SaRcaZm - V8 - Start
TeamFortressViewport::~TeamFortressViewport (void)
{
	delete m_pMainMenu;
//	delete m_pTeamMenu;
//	delete m_pClassMenu;
...
CommandButton *TeamFortressViewport::CreateCustomButton( char *pButtonText, char *pButtonName )
{
	...
	if ( !strcmp( pButtonName, "!CHANGETEAM" ) )
	{
		...
		// Auto Assign button
		m_pTeamButtons[11] = new TeamButton(12, gHUD.m_TextMessage.BufferedLocaliseTextString( "#Team_AutoAssign" ), 0, BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y);
		m_pTeamButtons[11]->addActionSignal(new CMenuHandler_StringCommand( "jointeam 5" ));
		pMenu->AddButton( m_pTeamButtons[11] ); 

		// Spectate button
		m_pTeamButtons[12] = new SpectateButton( CHudTextMessage::BufferedLocaliseTextString( "#Menu_Spectate" ), 0, BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y, false);
		m_pTeamButtons[12]->addActionSignal(new CMenuHandler_StringCommand( "spectate" ));
		pMenu->AddButton( m_pTeamButtons[12] ); 
	}
	// ChangeClass
...
void TeamFortressViewport::UpdateSpectatorMenu()
{
	...
	// SaRcaZm - V5 - Start
	// Don't draw if we have the VGUI displayed
...
void TeamFortressViewport::ShowVGUIMenu( int iMenu )
{
	...
	// SaRcaZm - V8 - Start
	if ((iMenu == 10) && m_pCurrentMenu && (m_pCurrentMenu->GetMenuID () == 10))
	{
		HideTopMenu ();
		return;
	}
	...
	{
	case MENU_TEAM:			// 2
		pNewMenu = ShowTeamMenu(); // TDK HAXOR
	//	gEngfuncs.pfnClientCmd( "jointeam peugeot; changecar 206\n" ); // TDK only

		break;

	// Map Briefing removed now that it appears in the team menu
	case MENU_MAPBRIEFING:	// 4
		pNewMenu = CreateTextWindow( SHOW_MAPBRIEFING );
		break;

	case MENU_INTRO:
		// SaRcaZm - V3 - Start
		// This is called after a map change, so clear the skidmarks
		// SaRcaZm - V5 - Start
//		CRallyEffects::getSingleton()->freeSkidmarks();
		// SaRcaZm - V3 - End
		...
		// SaRcaZm - V7 - Start
		// Clear the VGUI commands
		//ClearCommands ();
		// SaRcaZm - V7 - End

		pNewMenu = ShowMainMenu ();
		// SaRcaZm - V5 - End
		break;
...
//		m_pClassMenu->Update();
	...
	{
		// SaRcaZm - V5 - Start
		// I always wanna bring the next menu up immediately
...
void TeamFortressViewport::HideTopMenu()
{
	...
	// SaRcaZm - V5 - Start
	// Set the mode back to normal if this is the last one
	if (!m_pCurrentMenu || (m_pCurrentMenu == m_pMp3Menu))
...
// MAIN MENU
//======================================================================================
CMenuPanel *TeamFortressViewport::ShowMainMenu ()
{
	if (gEngfuncs.pDemoAPI->IsPlayingback ())
		return NULL;
	...
	m_pMainMenu->Initialize ();
	m_pMainMenu->Reset ();
	m_pMainMenu->Update (m_szMOTD);
	return m_pMainMenu;
}

void TeamFortressViewport::CreateMainMenu ()
{
	m_pMainMenu = new CMessageWindowPanel (m_szMOTD, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pMainMenu->setParent (this);
...
{
	// Create the panel
	m_pMp3Menu = new CMP3MenuPanel(100, false, 0, 0, ScreenWidth, ScreenHeight);
...
int	TeamFortressViewport::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	if (m_iUser1 && gEngfuncs.Con_IsVisible() == false )
	{
		...
		// SaRcaZm - V7 - Start
		{
			...
			// Hack in the current team and car selections to enable the menu properly
			//char sz[80];
			//memset (sz, 0, 80);
			// SaRcaZm - V8 - Start
			//if (m_szModel && m_szModel[0])
			//{
			//	sprintf (sz, "jointeam %s; changecar %s", m_szTeam, m_szModel);
			//	AddCommand (sz);
			//}
			// SaRcaZm - V8 - End
	...
	if (m_pCurrentMenu && gEngfuncs.Con_IsVisible() == false)
	{
		...
		{
			// SaRcaZm - V7 - Start
			// FUCKING THING TOOK ME FOREVER TO FIND
			// This was hiding the main menu straight after I created it and SCREWING ME OVER
...
int TeamFortressViewport::MsgFunc_VGUIClasses(const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	//	strncpy( CurrentRallyTeam, READ_STRING(), sizeof(CurrentRallyTeam) );
	//	CurrentRallyTeam[ sizeof(CurrentRallyTeam) - 1 ] = '\0';

	CurrentRallyTeam = READ_SHORT();
/*	for (int i = 0; i <= 50; i++)
	{
		char szz[256];
		sprintf(szz, "%s\n", CurrentRallyTeam ); 

		gEngfuncs.pfnConsolePrint(szz);
	}*/
...
// SaRcaZm - V6 - Start
char compressed[MAX_TEXTFILE_LENGTH * 2];
int length = 0;


int TeamFortressViewport::MsgFunc_TextFiles( const char *pszName, int iSize, void *pbuf )
{
	if (m_iGotAllTextFiles)
	{
		memset(m_szAnimTxt, 0, MAX_TEXTFILE_LENGTH);
		memset(m_szCarInfoTxt, 0, MAX_TEXTFILE_LENGTH);
		memset(m_szCustomTxt, 0, MAX_TEXTFILE_LENGTH);
		length = 0;
	}

	BEGIN_READ( pbuf, iSize );

	int chunklen = READ_BYTE();
	m_iGotAllTextFiles = (chunklen < MAX_TEXTFILE_CHUNK);	// So we can check this value elsewhere

	char chunk[MAX_TEXTFILE_CHUNK];
	for (int i = 0; i < chunklen; i++)
		chunk[i] = READ_CHAR();

	if (length + chunklen + 1 < sizeof (compressed))//MAX_TEXTFILE_LENGTH * 2)
	{
		memcpy (&compressed[length], chunk, chunklen);
		length += chunklen;
	}
	else
		// Now we're screwed
	...
	if (m_iGotAllTextFiles)
	{
		if (!ZLibFuncs.ZLib_Uncompress)
			return 0;

		// Can uncompress it all now
		int uncompressedlength = MAX_TEXTFILE_LENGTH * 3;
		...
		if (ZLibFuncs.ZLib_Uncompress ((unsigned char *)uncompressed, (unsigned long *) &uncompressedlength,
							(const unsigned char *)compressed, (unsigned long) length) != Z_OK)
		{
			// Now we are screwed!
			return 0;
		}

		// The uncompress loses the '\0' off the end, so put it back on
		uncompressed[uncompressedlength] = '\0';

		strcpy (m_szMOTD, uncompressed);
		int tmpLen = strlen (uncompressed) + 1;

		strcpy (m_szAnimTxt, uncompressed + tmpLen);
		tmpLen += strlen (uncompressed + tmpLen) + 1;

		strcpy (m_szCarInfoTxt, uncompressed + tmpLen);
		tmpLen += strlen (uncompressed + tmpLen) + 1;

		strcpy (m_szCustomTxt, uncompressed + tmpLen);
...
int TeamFortressViewport::MsgFunc_ScoreInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		...
		g_PlayerExtraInfo[cl].deaths = 0;
		//g_PlayerExtraInfo[cl].playerclass = playerclass;
		g_PlayerExtraInfo[cl].playerclass = 0;
...
//		UpdateOnPlayerInfo();
		// This is the only thing that needs updating
		// Updating everything was causing massive lag
		if (m_pScoreBoard)
			m_pScoreBoard->Update();
	}
	// SaRcaZm - V8 - End
...
// SaRcaZm - V5 - Start
// Allow the individual VGUI's to add commands that will be executed upon hitting the Race button
#include "carinfo.h"
...
{
	int iLen = strlen (sz) - 1;	// Be careful, this has been truncated on purpose

	// Make sure there are quotes
	if ((*sz != '"' && *sz != '\'') || (sz[iLen] != '"' && sz[iLen] != '\''))
		return;
...
extern vec3_t vStats;
void TeamFortressViewport::AddCommand (const char *pszCmd)
{
	if (m_szCmd[0])
		strcat (m_szCmd, pszCmd);
	else
		strcpy (m_szCmd, pszCmd);

	// HACK!!
	if ((!strnicmp (pszCmd, "jointeam", 8)) || (!strnicmp (pszCmd, "changecar", 9)))
	{
		char szCar[30];
		memset (szCar, 0, 30);

		GetCommandParam ("changecar", szCar);
		if (stricmp (szCar, m_szModel))
		{
			for (int i = 0; i < 8; i++)
				vUpgrades[i] = Vector (0, 0, 0);
		}
	}
	if (!strnicmp (pszCmd, "buy", 3))
	{
		char szPart[20];
		strcpy (szPart, pszCmd + 4);
		RemoveQuotes (szPart);

		// Add the item information into our vUpgrades array
		CCarInfo *carinfo = new CCarInfo (gViewPort->m_szCustomTxt, strlen (gViewPort->m_szCustomTxt));
		char szMake[20];
		int i = 0;
		vec3_t vUpdates = Vector (0, 0, 0);
		...
		vUpgrades[iClass] = vUpdates;
	}

	// Make sure the last character is a semi-colon
	int len = strlen (m_szCmd);
	if (m_szCmd[len-1] != ';')
		m_szCmd[len] = ';';
}

void TeamFortressViewport::ExecuteCommands (void)
{
	// SaRcaZm - V8 - Start
	char szTeam[20], szModel[30];

	memset (szTeam, 0, 20);
	memset (szModel, 0, 30);

	// Save the team and model names for later use
	GetCommandParam ("jointeam", szTeam);
	GetCommandParam ("changecar", szModel);
	...
	// Make sure we send the jointeam / changecar messages when in spectator mode
	if (!stricmp (szModel, m_szModel) && !g_iUser1)
	{
		RemoveCommand ("jointeam");
		RemoveCommand ("changecar");
	}
	else
	{
		strcpy (m_szTeam, szTeam);
		strcpy (m_szModel, szModel);
	}
	// SaRcaZm - V8 - End

	if (m_szCmd[0])
	{
		gEngfuncs.pfnClientCmd (m_szCmd);
		//gEngfuncs.Con_Printf (m_szCmd);
	}

	memset (m_szCmd, 0, 256);
	sprintf (m_szCmd, "jointeam %s; changecar %s; ", szTeam, szModel);
}

// SaRcaZm - V7 - Start
void TeamFortressViewport::ClearCommands (void)
{
	memset (m_szCmd, 0, 256);
}
// SaRcaZm - V7 - End

bool TeamFortressViewport::SearchCommands (const char *pszCmd)
{
	char *szPos = m_szCmd;
	char sz[80];
	int ret;

	ret = sscanf (szPos, "%s", sz);
	while ((ret != EOF) && ret)
	{
		if (!stricmp (sz, pszCmd))
			return true;
		szPos += strlen (sz) + 1;
		ret = sscanf (szPos, "%s", sz);
	}
...
bool TeamFortressViewport::GetCommandParam (const char *pszCmd, char *pszArgs)
{
	// SaRcaZm - V7 - Start
	// Find the last command that matches this description
	char *szPos = m_szCmd;
	char sz[80];
	bool found = false;
	char *pszFirstArg = pszArgs;
	int ret, iLen = 0;
	...
	while ((ret != EOF) && ret)
	{
		...
		if (!stricmp (sz, pszCmd))
		{
			...
			// We have the command, so read everything till the end of the string of the ';'
			while ((*szPos != 0) && (*szPos != ';'))
			{
				*pszArgs = *szPos;
				pszArgs++;
			...
			// Clear out what is left of the string
			for (int i = iLen; i < iOldLen; i++)
			{
				*pszArgs = 0;
				pszArgs++;
			}
...
// SaRcaZm - V5 - End
// SaRcaZm - V6 - Start
// I HATE string handling....Next time Creme can do it!
bool TeamFortressViewport::RemoveCommand (const char *pszKeyWord)
{
	// SaRcaZm - V8 - Start
	char *szPos = m_szCmd;
	char *szStartCmd = szPos;
	int iOrigLen = strlen (m_szCmd);
	char sz[80];
	int index;
	...
	while (*szPos && *szPos != ';')
	{
		// If we run into a space outside quotes, end reading
		if (*szPos == ' ' && !bQuotes)
			break;
	...
	while (*sz)
	{
		if (!stricmp (sz, pszKeyWord))
		{
			// Found the keyword we want, so remove it from the list
			// Step backwards to the beginning of the command
			// I'm an idiot. Comparing *szStartCmd to *m_szCmd
			while ((szStartCmd != m_szCmd) && (*szStartCmd != ';'))
				szStartCmd--;
			if (*szStartCmd == ';')
				szStartCmd++;

			// Jump szPos to the end of the current command
			while (*szPos && *szPos != ';')
				szPos++;
			// Skip to the next word
			while (*szPos && (*szPos == ' ' || *szPos == ';'))
				szPos++;
			...
			// Now to reset the size of the string
			m_szCmd[iOrigLen - (szPos - szStartCmd)] = 0;

			// Update the main menu to reflect the removed command
...
// SaRcaZm - V8 - Start
void TeamFortressViewport::mp3Command (char *szCommand)
{
	if (m_pMp3Menu)
		m_pMp3Menu->Command (szCommand);	
}

void TeamFortressViewport::mp3Frame (double time)
{
	if (m_pMp3Menu)
		m_pMp3Menu->frame (time);
}
// SaRcaZm - V8 - End
