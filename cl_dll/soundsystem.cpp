...
// SaRcaZm - V3 - Start
// MP3 playing code thru FMOD
#include "../fmod/api/inc/fmod.h"
void StartMP3 (char *filename)
{
	FSOUND_STREAM *stream; // Declare stream variable

	FSOUND_SetOutput(FSOUND_OUTPUT_DSOUND); // Output via DirectSound
	FSOUND_SetBufferSize(200);
	FSOUND_SetDriver(0); // Select default DirectSound device

	FSOUND_Init(44100, 16, 0); // Initialise playback stream to 44k 16bit

	stream = FSOUND_Stream_OpenMpeg(filename, FSOUND_NORMAL | FSOUND_LOOP_NORMAL);  // Open file
	FSOUND_Stream_Play(FSOUND_FREE, stream); // Play .mp3 file 
}
