...
void DLLEXPORT IN_MouseEvent (int mstate)
{
	...
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "IN_MouseEvent");
...
//		}
		...
		{
// Crem			viewangles[PITCH] += m_pitch->value * mouse_y;
		...
		else
		{
			...
			{
// Crem				cmd->forwardmove -= m_forward->value * mouse_y;
			}
		}
...
===========
*/
float IN_JoyMove ( float frametime, usercmd_t *cmd )
{
	float	speed, aspeed;
	float	fAxisValue, ForwardAxis, fTemp;
	int		i;
	vec3_t viewangles;
	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		case AxisForward:
			...
			else
			{
				...
				{
					// Creme disabled this as its now handled in the main input routine.
					//
		...
		case AxisSide:
			...
			{
				// Crem cmd->sidemove += (fAxisValue * joy_sidesensitivity->value) * speed * cl_sidespeed->value;
			}
			break;
...
float IN_Move ( float frametime, usercmd_t *cmd)
{
	...
	JoyMove = IN_JoyMove ( frametime, cmd);
