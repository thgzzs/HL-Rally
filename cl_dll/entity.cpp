...
int DLLEXPORT HUD_AddEntity( int type, struct cl_entity_s *ent, const char *modelname )
{
//	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_AddEntity");
...
void DLLEXPORT HUD_TxferLocalOverrides ( struct entity_state_s *state, const struct clientdata_s *client )
{
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_TxferLocalOverrides");
...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_ProcessPlayerState");
...
HUD_TxferPredictionData
 ...
 Because we can predict an arbitrary number of frames before the server responds 
 with an update, we need to be able to copy client side prediction data in
 from the state that the server ack'd receiving, which can be anywhere along 
 the predicted frame path ( i.e., we could predict 20 frames into the future 
 and the server ack's up through 10 of those frames, so we need to copy persistent 
 client-side only state from the 10th predicted frame to the slot the server
 update is occupying.
=========================
*/
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_TxferPredictionData");
...
void DLLEXPORT HUD_CreateEntities( void )
{
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_CreateEntities");
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_StudioEvent");
	switch( event->event )
	{
	case 5001:
	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	Global_DLLEXPORT_Info(gEngfuncs.GetClientTime(), "HUD_TempEntUpdate");
	...
	while ( pTemp )
	{
		...
		if ( pTemp->flags & FTENT_DUSTSPRITE ) {

			float LifeLeft;

			if (pTemp->entity.baseline.scale == 0) {
				pTemp->entity.baseline.scale = pTemp->entity.curstate.scale;
				
				//start life time!
				pTemp->entity.baseline.fuser1 = life; 
			...
			// Work out the % of life left for calc's
			LifeLeft = life / pTemp->entity.baseline.fuser1;

			// Fade renderamt
			pTemp->entity.curstate.renderamt = pTemp->entity.baseline.renderamt * LifeLeft;

			// Reduce speed by lifetime %
			pTemp->entity.baseline.origin = pTemp->entity.baseline.vuser1 * LifeLeft;

			// Scale dust up
			pTemp->entity.curstate.scale = 0.01 
				+ ((pTemp->entity.baseline.fuser1 
				+ (-life / pTemp->entity.baseline.fuser1)) 
				* pTemp->entity.baseline.scale);

			//gEngfuncs.Con_DPrintf("         life: %f; ", life);
			//gEngfuncs.Con_DPrintf("     scale: %f\n", pTemp->entity.baseline.scale);

			// Check scale for problems with tiny sprites 
			// (they are displayed at full size!)
			if (pTemp->entity.curstate.scale < 0.00125) pTemp->entity.curstate.scale = 0.00125;
		...
		else
		{
			...
			{
				// Creme note: pTemp->entity.baseline.origin seems 
				// to hold the sprites VELOCITY!
